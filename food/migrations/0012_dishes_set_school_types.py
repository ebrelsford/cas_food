...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Give dishes a school_type by their meals' school_types"
        for dish in orm.Dish.objects.all():
            try:
                # try to set dish's school_type to one of it's meals' school_types
                school_type = dish.meal_set.all()[0].school_type
            except Exception:
                school_type = 'elementary'

            # it's 'elementary' by default, so don't bother
            if school_type == 'elementary': continue

            dish.school_type = school_type
            dish.save()

    def backwards(self, orm):
        "Give all dishes the default school_type (elementary)."
        orm.Dish.objects.all().update(school_type='elementary')
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 8, 15, 15, 52, 9, 312729)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 8, 15, 15, 52, 9, 312581)'}),
