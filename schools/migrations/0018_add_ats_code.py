...
class Migration(DataMigration):
    ...
    ats_code_pattern = re.compile(r'^(\d{2}\w\d{3})')
    ...
    def forwards(self, orm):
        "Add ATS codes"
        ds = DataSource(self.school_shp)
        school_layer = ds[0]
        for i, feature in enumerate(school_layer):
            m = self.ats_code_pattern.match(feature.get('ATS_CODE'))
            if not m:
                continue
            orm.School.objects.filter(id=(i + 1)).update(ats_code=m.group(1))

    def backwards(self, orm):
        "Remove ATS codes"
        orm.School.objects.all().update(ats_code=None)
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 4, 2, 2, 6, 47, 153139)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 4, 2, 2, 6, 47, 152989)'}),
